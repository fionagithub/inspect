      ...
      <div class="layout-padding">
        <div class="card" ref="tab-1">
          <div v-if="message">
            ...
            <div class="card-actions card-no-top-padding">
              <div class="text-line" v-for='(item, index) in message.tags' :key="index" >
                {{item}}
              </div>
            </div>
        ...
        <div class="card" ref="tab-3">
          <div class="list item-inset-delimiter no-border">
            <div class="item item-link multiple-lines" v-for="(val,index) in 8 " :key="index" >
...
<script>
  ...
  export default {
    ...
    watch:{
      $route(to, from){
        let route=to||from
        console.log('--jjj--', route)
        route&&route.params&&route.params.id&&this.getMessage(route.params.id)
      }
    },
    methods: {
      getMessage(id) {
        id=id||this.$route.params.id
        this.findMessages(id)
      },
